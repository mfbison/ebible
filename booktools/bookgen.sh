...
ColSwitch=($($LeftToRight && printf "%d %d" 0 1 || printf "%d %d" 1 0)) #order the left and right elements in css
BOOKCAP=(31170) #TODO
...
{
MAPKEY=0
awkBook=${EndoBook[$((10#${indexlinearray[$READKEY]::3}))]}
awkCH=$((10#${indexlinearray[$READKEY]:3:3})) #slicing serial number and turning it base 10
if $LeftToRight; then
VerseNO="[$((10#${indexlinearray[$READKEY]:6:3}))]"
else
VerseNO="]$([[ $((10#${indexlinearray[$READKEY]:6:3})) =~ (.?)(.?)(.) ]] && printf ${BASH_REMATCH[3]}${BASH_REMATCH[2]}${BASH_REMATCH[1]})["
fi
...
<style>
@font-face {
 font-family: '${BookMeta[28]##*#}'; /* Name to use in CSS (case-sensitive!) */
 src: url('../../$FontLink') format('${FontMime[${FontLink##*.}]}'); /* Path to .ttf file + format hint */
 font-weight: normal; /* Default weight (400) */
 font-style: normal; /* Default style (normal) */
}
@page {
 size: A4 landscape;
}
